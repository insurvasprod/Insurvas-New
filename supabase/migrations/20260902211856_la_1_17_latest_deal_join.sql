-- LA-1.17 performance fix: enrich only the bounded page with its latest deal
-- notes. The previous lateral lookup ran once for every filtered lead and could
-- become an N+1 plan immediately after bulk intake.
...
as $$
  ...
  latest_page_deals as materialized (
    select distinct on (df.lead_id) df.lead_id, df.notes
    from public.deal_flow df
    join page_base p on p.id = df.lead_id
    where df.tenant_id = p_tenant_id and df.partner_id = p_partner_id
    order by df.lead_id, df.updated_at desc
  ),
  page as materialized (
    select p.*, d.notes as outcome_note
    from page_base p
    left join latest_page_deals d on d.lead_id = p.id
  ),
  totals as (
    select
  ...
  select jsonb_build_object(
    'rows', coalesce((
      select jsonb_agg(jsonb_build_array(
        r.id, r.work_item_id, r.customer, r.submitted_at, r.updated_at,
