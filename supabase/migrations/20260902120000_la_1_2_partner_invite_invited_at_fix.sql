-- LA-1.2 follow-up: qualify the INSERT RETURNING expression because invited_at is also a
-- return-table column in PL/pgSQL.
create or replace function public.partner_invite_user(
  p_tenant_id uuid, p_partner_id uuid, p_name text, p_email text,
  p_role public.partner_user_role, p_token_hash text, p_expires_at timestamptz
)
...
as $$
declare v_user_id uuid; v_invited_at timestamptz;
begin
  perform 1 from public.partners p
   where p.id = p_partner_id and p.tenant_id = p_tenant_id and p.status <> 'offboarded' for update;
  if not found then raise exception 'partner_not_found_or_offboarded'; end if;
  if exists (select 1 from public.users u where u.email = lower(btrim(p_email))) then raise exception 'partner_user_email_exists'; end if;
  insert into public.users (name, email, status) values (btrim(p_name), lower(btrim(p_email)), 'active') returning id into v_user_id;
  insert into public.partner_users (id, tenant_id, partner_id, user_id, role, status)
    values (gen_random_uuid(), p_tenant_id, p_partner_id, v_user_id, p_role, 'active') returning partner_users.invited_at into v_invited_at;
