-- LA-1.4 follow-up: duplicate platform templates must retain field metadata.
...
begin
  ...
  select new_template_id, 1, s.stage_key, s.label, s.stage_type, s.color, s.sort_order from public.template_stages s where s.template_id = p_template_id and s.version = source_row.version;
  insert into public.template_forms (template_id, version, form_definition)
  select new_template_id, 1, tf.form_definition from public.template_forms tf where tf.template_id = p_template_id and tf.version = source_row.version;
