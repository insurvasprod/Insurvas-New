-- LA-1.13 performance follow-up: return the filter options beside the report.
-- The API already resolves the tenant from the session before calling this helper. Keeping the
-- options in the same server-side payload avoids three additional round trips on every page load.
