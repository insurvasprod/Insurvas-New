...
async function main() {
  ...
  try {
    ...
    const requeue = await db.from("lead_queue").insert({ tenant_id: tenantId, lead_id: lead.id, partner_id: partnerId, product_line: "term_life", pipeline_id: lead.pipeline_id, stage_id: lead.stage_id }); check("the missing work item can be repaired without a second lead", !requeue.error);
