...
async function main() {
  ...
  const entitlement = { plan_code: "qa", plan_version: 1, status: "active", access: "full", computed_at: new Date().toISOString(), features: ["publisher_records"], meters: {}, limits: { max_seats: 3, max_publishers: 2, max_marketing_partners: 2, max_affiliates: 2, max_buffer_seats: null, max_partner_users: null } };
  ...
  try {
    ...
    check("partner details can be updated", updated.status === 200, `${updated.status} ${JSON.stringify(await updated.clone().json().catch(() => null))}`);
