...
async function main() {
  ...
  try {
    ...
    const hidden = await api("/api/partner/leads", portal, { method: "POST", ...json({ product_code: "term_life", values: { contact_preference: "Phone", email_address: "hidden@example.com" } }) }); check("hidden conditional fields are rejected server-side", hidden.status === 400);
    const submitValues = valuesFor(liveForm.template.template); submitValues.contact_preference = "Email"; submitValues.email_address = "qa@example.com"; const submitted = await api("/api/partner/leads", portal, { method: "POST", ...json({ product_code: "term_life", values: submitValues }) }); const submittedBody = await submitted.json().catch(() => null); const remainingDrafts = await db.from("form_drafts").select("id", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("partner_id", partnerId); check("a valid partner form submits a lead and clears its draft", submitted.status === 201 && remainingDrafts.count === 0, `status ${submitted.status}, error ${submittedBody?.error ?? "none"}`);
    const hostile = await api("/api/partner/leads", portal, { method: "POST", ...json({ product_code: "term_life", values: { contact_preference: "Email", email_address: "<script>alert(1)</script>" } }) }); check("hostile free-text and invalid email input is rejected", hostile.status === 400);
