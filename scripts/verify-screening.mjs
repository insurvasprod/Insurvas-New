...
async function main() {
  ...
  try {
    ...
    const submitted = await api("/api/partner/leads", portal, { method: "POST", ...json({ product_code: "term_life", submission_id: submissionId, screening_warning_acknowledged: true, values: dncValues }) }); const submittedBody = await submitted.json();
    ...
    check("both screening vendors unavailable fails closed and creates no lead", unavailable.status === 503 && unavailableBody.code === "unavailable" && unavailableBody.error === "Screening could not be completed. Do not treat this number as safe." && unavailableAfter.count === unavailableBefore.count && unavailableAudit.data?.length === 1, JSON.stringify({ status: unavailable.status, body: unavailableBody, before: unavailableBefore.count, after: unavailableAfter.count, audit: unavailableAudit.data, auditError: unavailableAudit.error?.message }));
    ...
    const replay = await api("/api/partner/leads", portal, { method: "POST", ...json({ product_code: "term_life", submission_id: submissionId, screening_warning_acknowledged: true, values: dncValues }) }); const replayBody = await replay.json(); const replayUsage = await db.from("usage_events").select("id", { count: "exact", head: true }).eq("tenant_id", tenantId); const replayResult = await db.from("screening_results").select("id").eq("tenant_id", tenantId).eq("phone_digits", "6025550101");
