...
async function main() {
  ...
  try {
    ...
    const replay = await api("/api/partner/leads", portal, { method: "POST", ...json({ product_code: "term_life", submission_id: submissionId, values: dncValues }) }); const replayBody = await replay.json(); const replayUsage = await db.from("usage_events").select("id").eq("tenant_id", tenantId); const replayResult = await db.from("screening_results").select("id").eq("tenant_id", tenantId).eq("phone_digits", "6025550101");
    check("same number and submission within TTL replays without another vendor call or credit", replay.status === 200 && replayBody.replayed === true && replayUsage.data?.length === 2 && replayResult.data?.length === 1);
