...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const { id } = await params; const template = await updateTenantTemplateCopy(auth.context.tenantId, id, { name: body.name, description: body.description ?? null, fields: body.fields, stages: body.stages, form_definition: body.form_definition }); await audit({ actorType: "tenant", actorId: auth.context.userId, action: "tenant.form_definition_saved", targetType: "tenant_template", targetId: id, metadata: { fieldCount: body.fields.length, sectionCount: body.form_definition.sections.length }, request }); return NextResponse.json({ template }); }
