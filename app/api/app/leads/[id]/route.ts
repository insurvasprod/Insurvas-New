...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!body?.values || !body.stage_key) return NextResponse.json({ error: "Lead values and stage are required" }, { status: 400 });
  try {
    const template = await getAgentTemplate(auth.context.tenantId, auth.context.userId);
    const lead = await updateAgentLead(auth.context.tenantId, (await params).id, template, body.values, body.stage_key);
