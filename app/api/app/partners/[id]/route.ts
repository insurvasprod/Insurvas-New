...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    if (parsed.data.action === "update") {
      const partner = await updatePartner(auth.context.tenantId, id, parsed.data, auth.entitlement.limits);
    ...
    if (message.includes("partner_limit_reached") && limit) return NextResponse.json({ error: `Your plan has reached ${limit[1]} (${limit[2]} of ${limit[3]}). Upgrade to activate this partner.`, code: "limit_reached", limitKey: limit[1], usage: Number(limit[2]), limit: Number(limit[3]), upgrade: true }, { status: 403 });
