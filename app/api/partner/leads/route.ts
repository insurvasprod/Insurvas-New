...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const phoneField = template.template.fields.find((field) => field.type === "phone" && (field.field_key === "phone" || field.field_key === "phone_number")) ?? template.template.fields.find((field) => field.type === "phone");
    ...
    await writePartnerIntakeArtifacts({ tenantId: auth.context.tenantId, partnerId: auth.context.partnerId, userId: auth.context.userId, partnerTimezone: auth.context.partnerTimezone, submissionId: body.submission_id, lead, request });
