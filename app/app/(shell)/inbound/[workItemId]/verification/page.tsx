...
export default async function VerificationPage({ params }: { params: Promise<{ workItemId: string }> }) {
  ...
  if (!["owner", "producer"].includes(guard.role)) return <RoleGateNotice featureLabel="Inbound transfers" detail="Only owners and producers can verify an inbound transfer." />;
  const { workItemId } = await params;
  return <VerificationPanel workItemId={workItemId} readOnly={guard.entitlement.status === "suspended" || guard.entitlement.status === "paused"} />;
}
