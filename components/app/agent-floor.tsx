...
type FloorData = { waiting: Lead[]; onCalls: Call[]; available: Member[]; members: Member[]; pendingHandoffs: Handoff[]; realtimeTopic: string; generatedAt: string };
...
function waitTone(queuedAt: string, now: number) {
  const seconds = Math.max(0, Math.floor((now - new Date(queuedAt).getTime()) / 1000));
  return seconds >= 300 ? "border-red-500/60 bg-red-500/5" : seconds >= 120 ? "border-amber-500/60 bg-amber-500/5" : "border-border";
}
...
function LeadCard({ item, now, readOnly, onClaim, onNudge, saving }: { item: Lead; now: number; readOnly: boolean; onClaim: (id: string) => void; onNudge: (id: string) => void; saving: string | null }) {
  return <Card className={`gap-3 ${waitTone(item.queuedAt, now)}`}>
...
export function AgentFloor({ currentUserId, readOnly }: { currentUserId: string; readOnly: boolean }) {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6">
    ...
    <section aria-labelledby="waiting-heading" className="space-y-3"><div className="flex items-center justify-between gap-3"><h2 id="waiting-heading" className="text-lg font-bold">Waiting <span className="text-muted-foreground">{floor.waiting.length}</span></h2><p className="text-xs text-muted-foreground">Amber after 2 minutes · red after 5</p></div>{floor.waiting.length === 0 ? <Card><CardContent className="p-5 text-sm text-muted-foreground">No transfers are waiting. New qualified inbound calls will appear here automatically.</CardContent></Card> : <div className="grid gap-3 xl:grid-cols-2">{floor.waiting.map((item) => <LeadCard key={item.id} item={item} now={now} readOnly={readOnly} onClaim={(id) => void claim(id)} onNudge={(id) => void nudge(id)} saving={saving} />)}</div>}</section>
