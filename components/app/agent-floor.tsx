...
type FloorData = { waiting: Lead[]; onCalls: Call[]; available: Member[]; members: Member[]; pendingHandoffs: Handoff[]; callbacks: Array<{ id: string; leadId: string; customerName: string; customerTime: string; customerTimezone: string; note: string | null; isOverdue: boolean; isDueToday: boolean }>; realtimeTopic: string; generatedAt: string; waitThresholds: { amberSeconds: number; redSeconds: number } };
...
export function AgentFloor({ currentUserId, readOnly }: { currentUserId: string; readOnly: boolean }) {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6">
    ...
    <section aria-labelledby="callbacks-heading" className="space-y-3"><div className="flex items-center justify-between gap-3"><h2 id="callbacks-heading" className="text-lg font-bold">Callbacks due <span className="text-muted-foreground">{floor.callbacks.length}</span></h2><Button size="sm" variant="outline" asChild><Link href="/app/callbacks">Open calendar</Link></Button></div>{floor.callbacks.length === 0 ? <Card><CardContent className="p-5 text-sm text-muted-foreground">No callbacks are due today.</CardContent></Card> : <div className="grid gap-3 xl:grid-cols-2">{floor.callbacks.map((callback) => <Card key={callback.id}><CardContent className="space-y-2 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{callback.customerName}</p><p className="text-sm text-muted-foreground">{callback.customerTime} · {callback.customerTimezone}</p></div><Badge variant={callback.isOverdue ? "destructive" : "secondary"}>{callback.isOverdue ? "Overdue" : "Due"}</Badge></div><p className="text-sm text-muted-foreground">{callback.note || "No callback note."}</p><Button size="sm" variant="outline" asChild><Link href={`/app/leads/${callback.leadId}`}>Open lead</Link></Button></CardContent></Card>)}</div>}</section>
