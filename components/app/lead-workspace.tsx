...
function PipelineBoard({ stages, leads, readOnly, onStageChange }: { stages: PipelineStage[]; leads: Lead[]; readOnly: boolean; onStageChange: (lead: Lead, stage: string) => void }) {
  return <div className="grid gap-3 xl:grid-cols-4">{stages.filter((stage) => !stage.is_archived).map((stage) => <Card key={stage.id} className="min-w-0"><CardHeader className="border-t-4 pb-3" style={{ borderTopColor: stage.color }}><CardTitle className="text-sm">{stage.name}<span className="ml-2 text-xs font-normal text-muted-foreground">{leads.filter((lead) => lead.stage_id === stage.id).length}</span></CardTitle></CardHeader><CardContent className="space-y-2">{leads.filter((lead) => lead.stage_id === stage.id).map((lead) => <div key={lead.id} className="rounded-md border p-3"><p className="truncate text-sm font-medium">{String(lead.values.full_name ?? lead.values.name ?? "Unnamed lead")}</p><p className="mt-1 truncate text-xs text-muted-foreground">{Object.values(lead.values).filter(Boolean).slice(1, 3).map(String).join(" · ")}</p>{lead.screening_warning && <p role="status" className="mt-2 rounded-md border border-amber-500/40 bg-amber-500/10 px-2 py-1.5 text-xs text-amber-800">⚠ {lead.screening_warning}</p>}<select aria-label={`Move lead to stage`} disabled={readOnly} className="border-input bg-background mt-2 h-8 w-full rounded-md border px-2 text-xs" value={lead.stage_id} onChange={(event) => onStageChange(lead, event.target.value)}>{stages.filter((item) => !item.is_archived).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>)}{!leads.some((lead) => lead.stage_id === stage.id) && <p className="text-xs text-muted-foreground">No leads here yet.</p>}</CardContent></Card>)}</div>;
}
