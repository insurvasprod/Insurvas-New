...
type Lead = { id: string; pipeline_id: string; stage_id: string; values: Record<string, unknown>; screening_outcome?: string | null; screening_warning?: string | null; screening_checked_at?: string | null; created_at: string; updated_at: string };
type PageData = { template: { assignment: { template_version: number }; template: TemplateRow; latest: { version: number; name: string } | null }; pipelines: Array<{ id: string; name: string; partner_type: string; stages: PipelineStage[] }>; leads: Lead[]; readOnly: boolean };
...
function LeadForm({ template, stages, readOnly, onCreated }: { template: TemplateRow; stages: PipelineStage[]; readOnly: boolean; onCreated: () => void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setSaving(true);
    const response = await fetch("/api/app/leads", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ values, stage_id: stage }) });
  ...
  return <Card><CardHeader><CardTitle className="text-base">New {template.product_name} lead</CardTitle><p className="text-sm text-muted-foreground">Form version {template.definition_version}. Currency values are stored as integer cents. {draftStatus}</p></CardHeader><CardContent><form onSubmit={submit} className="space-y-5">
    ...
    <div className="space-y-1.5"><Label htmlFor="lead-stage">Starting stage</Label><select id="lead-stage" className="border-input bg-background h-9 w-full rounded-md border px-3 text-sm" value={stage} onChange={(event) => setStage(event.target.value)}>{stages.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
    <Button type="submit" disabled={readOnly || saving}>{readOnly ? "Read-only account" : saving ? "Adding…" : "Add lead"}</Button>
  </form></CardContent></Card>;
}

function PipelineBoard({ stages, leads, readOnly, onStageChange }: { stages: PipelineStage[]; leads: Lead[]; readOnly: boolean; onStageChange: (lead: Lead, stage: string) => void }) {
  return <div className="grid gap-3 xl:grid-cols-4">{stages.filter((stage) => !stage.is_archived).map((stage) => <Card key={stage.id} className="min-w-0"><CardHeader className="border-t-4 pb-3" style={{ borderTopColor: stage.color }}><CardTitle className="text-sm">{stage.name}<span className="ml-2 text-xs font-normal text-muted-foreground">{leads.filter((lead) => lead.stage_id === stage.id).length}</span></CardTitle></CardHeader><CardContent className="space-y-2">{leads.filter((lead) => lead.stage_id === stage.id).map((lead) => <div key={lead.id} className="rounded-md border p-3"><Link href={`/app/leads/${lead.id}`} className="block truncate text-sm font-medium underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{String(lead.values.full_name ?? lead.values.name ?? "Unnamed lead")}</Link><p className="mt-1 truncate text-xs text-muted-foreground">{Object.values(lead.values).filter(Boolean).slice(1, 3).map(String).join(" · ")}</p>{lead.screening_warning && <p role="status" className="mt-2 rounded-md border border-amber-500/40 bg-amber-500/10 px-2 py-1.5 text-xs text-amber-800">⚠ {lead.screening_warning}</p>}<select aria-label={`Move lead to stage`} disabled={readOnly} className="border-input bg-background mt-2 h-8 w-full rounded-md border px-2 text-xs" value={lead.stage_id} onChange={(event) => onStageChange(lead, event.target.value)}>{stages.filter((item) => !item.is_archived).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>)}{!leads.some((lead) => lead.stage_id === stage.id) && <p className="text-xs text-muted-foreground">No leads here yet.</p>}</CardContent></Card>)}</div>;
}
...
export function LeadWorkspace() {
  ...
  async function updateStage(lead: Lead, stage: string) { const response = await fetch(`/api/app/leads/${lead.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ values: lead.values, stage_id: stage }) }); if (!response.ok) { const body = await response.json().catch(() => null); toast.error(body?.error ?? "Could not move lead"); return; } toast.success("Lead moved"); void load(query); }
  ...
  const runtimeStages = data.pipelines.flatMap((pipeline) => pipeline.stages);
  return <div className="mx-auto max-w-7xl space-y-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-extrabold tracking-tight">Lead workspace</h1><p className="mt-1 text-sm font-medium text-muted-foreground">{template.name} · {template.product_name} · pinned version {data.template.assignment.template_version}</p></div><div className="flex gap-2">{data.template.latest && <Button variant="outline" onClick={() => void updateTemplate()}>Update to v{data.template.latest.version}</Button>}<Button asChild variant="outline"><a href={`/api/app/leads/export${exportParams}`}>Export CSV</a></Button></div></div>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(280px,360px)_1fr]"><LeadForm template={template} stages={runtimeStages.filter((stage) => stage.pipeline_id === data.pipelines.find((pipeline) => pipeline.partner_type === "marketing")?.id)} readOnly={data.readOnly} onCreated={() => void load(query)} /><div className="space-y-3"><div className="flex items-center justify-between"><h2 className="text-lg font-bold">Pipeline</h2><Badge variant="outline">{data.leads.length} leads</Badge></div><PipelineBoard stages={runtimeStages} leads={data.leads} readOnly={data.readOnly} onStageChange={updateStage} /></div></div>
  </div>;
}
