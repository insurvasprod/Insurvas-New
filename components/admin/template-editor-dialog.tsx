...
function previewField(field: TemplateField) {
  return <div key={field.field_key} className="rounded-md border bg-background p-2"><p className="text-xs font-medium">{field.label}{field.is_required && <span className="text-destructive"> *</span>}</p><p className="mt-1 text-xs text-muted-foreground">{TEMPLATE_FIELD_TYPE_LABELS[field.type]}</p></div>;
}
