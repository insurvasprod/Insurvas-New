...
export function ComplianceVendorsTable({ initialVendors }: { initialVendors: ComplianceVendor[] }) {
  ...
  return <div className="space-y-5">
    {dialingBlocked && <div role="alert" className="flex items-start gap-3 rounded-lg border-2 border-[var(--color-danger)]/50 bg-[var(--color-danger)]/5 p-4 text-sm text-[var(--color-danger)]"><ShieldAlert className="mt-0.5 size-5 shrink-0" /><div><p className="font-bold">Dialing is blocked</p><p className="mt-1">{DNC_BLOCK_MESSAGE}</p></div></div>}
    ...
    <div className={tableShell}><Table><TableHeader><TableRow className={tableHeaderRow}><TableHead className={tableHeadCell}>Vendor</TableHead><TableHead className={tableHeadCell}>Type</TableHead><TableHead className={tableHeadCell}>Endpoint</TableHead><TableHead className={tableHeadCell}>Health (24h)</TableHead><TableHead className={tableHeadCell}>Cost</TableHead><TableHead className={`${tableHeadCell} w-10`} /></TableRow></TableHeader><TableBody>
      {vendors.length === 0 && <TableRow><TableCell colSpan={6} className="h-20 text-center text-sm text-muted-foreground">No vendors registered yet &mdash; which is why dialing is blocked. Add a DNC scrub vendor to unblock it.</TableCell></TableRow>}
