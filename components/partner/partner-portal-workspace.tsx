...
function PartnerLeadForm({ productCode }: { productCode: string }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); const response = await fetch("/api/partner/leads", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ product_code: productCode, values, submission_id: submissionId.current }) }); const body = await response.json().catch(() => null); setSaving(false); if (!response.ok) { toast.error(body?.error ?? "Could not submit lead"); return; } setValues({}); submissionId.current = crypto.randomUUID(); setStatus(body?.replayed ? "Already submitted" : "Submitted"); toast.success(body?.replayed ? "This lead was already submitted" : "Lead submitted to the agent"); }
