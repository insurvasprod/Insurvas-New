...
import { partnerTypeForLead, resolveRuntimeStage, assertUuid } from "@/lib/pipelines/service";
...
export async function listAgentLeads(tenantId: string, template: AgentTemplate, search: string, filterField: string, filterValue: string, sortField: string, direction: "asc" | "desc") {
  ...
  const { data, error } = await getSupabaseServiceClient().from("agent_leads").select("id, product_line, pipeline_id, stage_id, values, screening_outcome, screening_warning, screening_checked_at, created_at, updated_at").eq("tenant_id", tenantId).eq("tenant_template_id", template.tenant_template_id).order("created_at", { ascending: false }); if (error) throw new Error(`Could not load leads: ${error.message}`);
...
export async function createAgentLead(tenantId: string, userId: string, template: AgentTemplate, values: unknown, stageKey?: string) {
  ...
  if (stageKey && /^[0-9a-f-]{36}$/i.test(stageKey)) {
    assertUuid(stageKey, "stage id"); const { data: runtimeStage } = await getSupabaseServiceClient().from("pipeline_stages").select("id, pipeline_id, is_archived").eq("id", stageKey).eq("is_archived", false).maybeSingle(); if (!runtimeStage) throw new Error("Choose a valid pipeline stage"); const { data: pipeline } = await getSupabaseServiceClient().from("pipelines").select("id").eq("id", runtimeStage.pipeline_id).eq("tenant_id", tenantId).maybeSingle(); if (!pipeline) throw new Error("Choose a valid pipeline stage"); stage = { pipelineId: pipeline.id, stage: runtimeStage };
  } else stage = await resolveRuntimeStage(tenantId, stageKey ?? template.template.stages[0]?.stage_key ?? "new");
  const { data, error } = await getSupabaseServiceClient().from("agent_leads").insert({ tenant_id: tenantId, tenant_template_id: template.tenant_template_id, template_id: template.assignment.template_id, template_version: template.assignment.template_version, definition_version: template.assignment.definition_version, product_line: template.template.product_code, pipeline_id: stage.pipelineId, stage_id: stage.stage.id, values: normalized.values as Json, created_by: userId }).select("id, product_line, pipeline_id, stage_id, values, created_at, updated_at").single(); if (error || !data) throw new Error(error?.message ?? "Could not create lead"); return data;
}
export async function updateAgentLead(tenantId: string, leadId: string, template: AgentTemplate, values: unknown, stageIdentifier: string) {
  const normalized = normalizeFormValues(template.template.fields, template.template.form_definition, values); if (normalized.error) throw new Error(normalized.error);
  let pipelineId: string; let stageId: string;
  if (stageIdentifier && /^[0-9a-f-]{36}$/i.test(stageIdentifier)) {
    assertUuid(stageIdentifier, "stage id"); const { data: stage } = await getSupabaseServiceClient().from("pipeline_stages").select("id, pipeline_id").eq("id", stageIdentifier).maybeSingle(); if (!stage) throw new Error("Choose a valid pipeline stage"); pipelineId = stage.pipeline_id; stageId = stage.id;
    const { data: pipeline } = await getSupabaseServiceClient().from("pipelines").select("id").eq("id", pipelineId).eq("tenant_id", tenantId).maybeSingle(); if (!pipeline) throw new Error("Choose a valid pipeline stage");
  } else { const stage = await resolveRuntimeStage(tenantId, stageIdentifier); pipelineId = stage.pipelineId; stageId = stage.stage.id; }
  const { data, error } = await getSupabaseServiceClient().from("agent_leads").update({ values: normalized.values as Json, product_line: template.template.product_code, definition_version: template.assignment.definition_version, pipeline_id: pipelineId, stage_id: stageId }).eq("id", leadId).eq("tenant_id", tenantId).eq("tenant_template_id", template.tenant_template_id).select("id, product_line, pipeline_id, stage_id, values, created_at, updated_at").single(); if (error || !data) throw new Error(error?.message ?? "Lead not found");
  await getSupabaseServiceClient().from("lead_queue").update({ pipeline_id: pipelineId, stage_id: stageId }).eq("lead_id", leadId).eq("tenant_id", tenantId); await getSupabaseServiceClient().from("deal_flow").update({ pipeline_id: pipelineId, stage_id: stageId }).eq("lead_id", leadId).eq("tenant_id", tenantId); return data;
}
...
export async function createPartnerLead(
  ...
  const normalized = normalizeFormValues(template.template.fields, template.template.form_definition, values); if (normalized.error) throw new Error(normalized.error);
  const supabase = getSupabaseServiceClient();
  const selected = "id, product_line, partner_id, submission_id, pipeline_id, stage_id, values, affiliate_link_id, affiliate_campaign, screening_outcome, screening_warning, screening_warning_acknowledged, screening_warning_acknowledged_at, duplicate_override_justification, duplicate_override_by, duplicate_override_at, screening_checked_at, created_at, updated_at";
  ...
  const partnerType = await partnerTypeForLead(tenantId, partnerId); const stage = await resolveRuntimeStage(tenantId, template.template.stages[0]?.stage_key ?? "new", partnerType);
...
export function csvForLeads(fields: TemplateField[], stages: Array<{ id: string; name: string }>, leads: Array<{ stage_id: string; values: Record<string, unknown> }>) { const escape = (value: unknown) => { const text = Array.isArray(value) ? value.join(", ") : value === null || value === undefined ? "" : String(value); const safe = /^[=+\-@]/.test(text) ? `'${text}` : text; return `"${safe.replaceAll('"', '""')}"`; }; return [["stage", ...fields.map((field) => field.label)].map(escape).join(","), ...leads.map((lead) => [stages.find((stage) => stage.id === lead.stage_id)?.name ?? "Unknown stage", ...fields.map((field) => lead.values[field.field_key])].map(escape).join(","))].join("\r\n") + "\r\n"; }
