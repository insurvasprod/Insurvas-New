...
function validateCopy(fields: TemplateField[], stages: TemplateStage[], form: TemplateFormDefinition) {
  if (!fields.length || !stages.length || !form.sections.length) return "A template copy needs fields, pipeline stages and a form";
  const keys = new Set<string>(); for (const field of fields) { if (!TEMPLATE_KEY_PATTERN.test(field.field_key) || keys.has(field.field_key)) return "Field keys must be unique lowercase names"; if (!TEMPLATE_FIELD_TYPES.includes(field.type)) return `Unsupported field type for ${field.label}`; keys.add(field.field_key); }
  const stageKeys = new Set<string>(); for (const stage of stages) { if (!TEMPLATE_KEY_PATTERN.test(stage.stage_key) || stageKeys.has(stage.stage_key)) return "Stage keys must be unique lowercase names"; if (!TEMPLATE_STAGE_TYPES.includes(stage.stage_type) || !/^#[0-9a-fA-F]{6}$/.test(stage.color)) return `Invalid stage ${stage.label}`; stageKeys.add(stage.stage_key); } return null;
}
...
export async function updateTenantTemplateCopy(tenantId: string, id: string, input: { name: string; description: string | null; fields: TemplateField[]; stages: TemplateStage[]; form_definition: TemplateFormDefinition }) {
  const validation = validateCopy(input.fields, input.stages, input.form_definition); if (validation) throw new Error(validation); if (!input.name.trim()) throw new Error("Template name is required");
