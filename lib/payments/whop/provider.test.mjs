...
test("createPlan stamps our plan identity as metadata and converts the cycle to days", async () => {
  ...
  // The recurring price goes in renewal_price. initial_price is an EXTRA upfront charge, so it
  // must be the setup fee (zero here) — setting it to the plan price double-charged the first
  // payment in the sandbox: $99 + $99 = $198.
  assert.equal(calls[0].body.renewal_price, 449.99);
  assert.equal(calls[0].body.initial_price, 0);
...
test("a setup fee, when there is one, goes to initial_price", async () => {
  const { provider, calls } = providerWith({ id: "plan_new" });
  await provider.createPlan({
    productId: "prod_1", priceCents: 9900, setupFeeCents: 5000, billingCycle: "monthly",
    ourPlanId: TENANT, planCode: "plan_a", planVersion: 1,
  });

  assert.equal(calls[0].body.renewal_price, 99);
  assert.equal(calls[0].body.initial_price, 50);
});
