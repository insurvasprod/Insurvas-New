...
test("a group with one visible child flattens to a plain link", () => {
  // platform_config can open no Customers or Billing screen and only one Monitoring one, so
  // Monitoring collapses to a plain "Audit log" link rather than a one-item accordion.
  const nodes = buildAdminNav("platform_config");
  assert.deepEqual(labels(nodes), ["Dashboard", "Audit log", "Platform"]);
