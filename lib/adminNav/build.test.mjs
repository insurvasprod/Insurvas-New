...
test("Subscriptions and Trials are grouped with Billing, not with Customers", () => {
  ...
  assert.deepEqual(labels(billing.links), [
    "Subscriptions", "Trials", "Invoices", "Refunds & credits", "Coupons", "Revenue", "Setup",
  ]);
});
...
test("a billing_admin gets the whole billing group but not the catalog", () => {
  ...
  // Six, not seven: payment Setup sits in this group but keeps the section registry's stricter
  // rule, so live provider credentials stay super_admin-only even here.
  assert.equal(findGroup(nodes, "billing").links.length, 6);
  assert.ok(!labels(findGroup(nodes, "billing").links).includes("Setup"));
...
test("Features is filed under Catalog, not Platform", () => {
  ...
  const nodes = buildAdminNav("super_admin");
  assert.deepEqual(labels(findGroup(nodes, "catalog").links), ["Plans", "Add-ons", "Features"]);
...
test("a group with one visible child flattens to a plain link", () => {
  // platform_config can open no Plans or Add-ons, so Catalog collapses to a plain "Features" link,
  // and only one Monitoring screen, so that collapses to "Audit log". Neither becomes a one-item
  // accordion.
  const nodes = buildAdminNav("platform_config");
  assert.deepEqual(labels(nodes), ["Dashboard", "Features", "Audit log", "Platform"]);
  assert.equal(findGroup(nodes, "catalog"), undefined, "one child should not be an accordion");
