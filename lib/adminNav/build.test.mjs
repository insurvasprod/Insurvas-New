...
test("renamed screens keep their original routes", () => {
  // Labels changed; URLs did not, so anything already linking to these keeps working.
  const platform = findGroup(buildAdminNav("super_admin"), "platform").links;
  const labelFor = (href) => platform.find((l) => l.href === href)?.label;

  assert.equal(labelFor("/admin/system"), "Maintenance");
  assert.equal(labelFor("/admin/compliance-sources"), "Compliance");
  assert.equal(labelFor("/admin/email"), "Mail Setup");

  for (const gone of ["System", "Compliance sources", "Email"]) {
    assert.ok(!platform.some((l) => l.label === gone), `${gone} is still in the sidebar`);
  }
});
