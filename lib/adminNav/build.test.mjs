...
test("a super_admin sees every screen, and none of them twice", () => {
  ...
  // 17 since the module-4 merge added SA-4.3's Configuration Center to the Platform group.
...
test("a group with one visible child flattens to a plain link", () => {
  // platform_config reaches few screens, so Catalog and Monitoring collapse to single links rather
  // than one-item accordions. Platform stays a real group: since the module-4 merge this role can
  // also reach the Configuration Center, giving it two children.
  const nodes = buildAdminNav("platform_config");
  assert.deepEqual(labels(nodes), ["Dashboard", "Features", "Audit log", "Platform"]);
  assert.equal(nodes.filter((n) => n.kind === "group").length, 1);
  assert.deepEqual(
    labels(findGroup(nodes, "platform").links),
    ["Configuration Center", "Legal"],
  );
});

test("a support_agent cannot reach the Configuration Center", () => {
  // It has no section they may open, so canAccessConfigurationCenter is false and the link is not
  // rendered — the same "no link beats a link to a 403" rule the rest of the nav follows.
  const hrefs = allLinks(buildAdminNav("support_agent")).map((l) => l.href);
  assert.ok(!hrefs.includes("/admin/configuration"), hrefs.join(", "));
});
