...
test("Subscriptions and Trials are grouped with Billing, not with Customers", () => {
  ...
  assert.deepEqual(labels(billing.links), [
    ...
    "Offers & discounts", "Revenue", "Setup",
  ]);
});
...
test("a billing_admin gets the whole billing group but not the catalog", () => {
  ...
  // Seven of eight: Offers is theirs, but payment Setup keeps the section registry's stricter
  // rule, so live provider credentials stay super_admin-only even inside their own group.
  assert.equal(findGroup(nodes, "billing").links.length, 7);
  assert.ok(labels(findGroup(nodes, "billing").links).includes("Offers & discounts"));
...
test("a group with one visible child flattens to a plain link", () => {
  ...
  assert.deepEqual(labels(nodes), ["Dashboard", "Catalog", "Audit log", "Platform"]);
